...
def countNum(xDict):
    ...
    xLen = len(x)
    # 以p覆盖率确定句子长度
    pSentLen = x[int(0.9*xLen) - 1]
    x = np.sort(ReviewLenList)
    # 统计有多少个评论
    xLen = len(x)
    # 以p覆盖率确定句子长度
    pReviewLen = x[int(0.9*xLen) - 1]
...
if __name__ == '__main__':
    ...
    for i in data.values:
        ...
        if user_reviews_dict.__contains__(i[0]):
            ...
            user_summary_dict[i[0]].append(str_summary)
            user_iid_dict[i[0]].append(i[1])

        else:
            user_reviews_dict[i[0]] = [str_review]
            review_summary_dict[i[0]] = [str_review_summary]
            user_summary_dict[i[0]] = [str_summary]
            user_iid_dict[i[0]] = [i[1]]

        if item_reviews_dict.__contains__(i[1]):
            item_reviews_dict[i[1]].append(str_review)
            item_summary_dict[i[1]].append(str_summary)
            item_uid_dict[i[1]].append(i[0])
        else:
            item_reviews_dict[i[1]] = [str_review]
            item_summary_dict[i[1]] = [str_summary]
            item_uid_dict[i[1]] = [i[0]]

    # np.save("./{}/train/npy/user_item2id.npy".format(save_folder), user_iid_dict)
    # np.save("./{}/train/npy/item_user2id.npy".format(save_folder), item_uid_dict)
    ...
    for i in range(userNum):
        ...
        summaryList = user_summary_dict[i]
        ...
        for summary in summaryList:
            summary2index = []
            # wordTokens = text_to_word_sequence(summary)
            wordTokens = summary.strip().split()
            if len(wordTokens) >= minSentlen:
                k = 0
                if len(wordTokens) > maxSentLen_summary:
                    u_summaryLen.append(maxSentLen_summary)
                else:
                    u_summaryLen.append(len(wordTokens))
                for _, word in enumerate(wordTokens):
                    if k < maxSentLen_summary:
                        summary2index.append(word_index[word])
                        k = k + 1
                    else:
                        break
            else:
                a_count += 1
                u_summaryLen.append(1)
            if len(summary2index) < maxSentLen_summary:
                summary2index = summary2index + [0] * (maxSentLen_summary - len(summary2index))
            u_summaryList.append(summary2index)
        if a_count > 0:
            print("{} has less minlen".format(a_count))
        if len(u_summaryList) == 0:
            # print("第{}个用户有问题".format(i))
            u_summaryList.append([0] * maxSentLen_summary)
            u_summaryLen.append(1)
        ...
        userSummary2Index.append(padding_text(u_summaryList, u_pReviewLen))
        ...
        # dataList.append(u_reviewList)
        # dataList.append(u_reviewLen)
        # userReview2Index.append(dataList)
    ...
    np.save("./{}/train/npy/userSummary2Index.npy".format(save_folder), userSummary2Index)
    ...
    for i in range(itemNum):
        ...
        summaryList = item_summary_dict[i]
        ...
        for summary in summaryList:
            ...
            if len(wordTokens) >= minSentlen:
                k = 0
                if len(wordTokens) > maxSentLen_summary:
                    i_summaryLen.append(maxSentLen_summary)
                else:
                    i_summaryLen.append(len(wordTokens))
                for _, word in enumerate(wordTokens):
                    if k < maxSentLen_summary:
                        summary2index.append(word_index[word])
                        k = k + 1
                ...
                i_summaryLen.append(1)
            if len(summary2index) < maxSentLen_summary:
                summary2index = summary2index + [0]*(maxSentLen_summary-len(summary2index))
            i_summaryList.append(summary2index)
        if len(i_summaryList) == 0:
            # print("第{}个有问题".format(i))
            i_summaryList.append([0]*maxSentLen_summary)
            i_summaryLen.append(1)
        # dataList_summary.append(i_summaryList)
        # dataList_summary.append(i_summaryLen)
        # itemSummary2Index.append(dataList_summary)
        itemSummary2Index.append(padding_text(i_summaryList, i_pReviewLen))
    ...
    np.save("./{}/train/npy/itemSummary2Index.npy".format(save_folder), itemSummary2Index)
